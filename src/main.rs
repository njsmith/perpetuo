...
use perpetuo::log::{convert_stack, log, log_with_details, Severity, StallDetails};
...
fn watch_process(pid: u32, cli: &Cli) -> Result<()> {
    ...
    log(
        Severity::Info,
        &format!("Attempting to monitor pid {pid}..."),
        &additional_info,
        json_mode,
    );
    // let mut proc = loop {
    ...
    log(
        Severity::Info,
        &format!("Successfully monitoring pid {pid}"),
        &additional_info,
        json_mode,
    );
